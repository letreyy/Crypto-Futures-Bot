...
export class ScoringEngine {
    static calculate(ctx: StrategyContext, candidate: StrategySignalCandidate): { score: number, label: string } {
        ...
        // Weight adjustments
        if (ctx.regime.type === MarketRegimeType.TREND) score += config.weights.regimeAlignment;
        if (ctx.indicators.volumeSma > 0 && ctx.candles[ctx.candles.length - 1].volume > ctx.indicators.volumeSma * 1.5) score += config.weights.volumeSpike;
        ...
        if (ctx.funding) {
            ...
            if (ctx.funding.rate > 0.0001 && candidate.direction === 'SHORT') score += 5;
            if (ctx.funding.rate < -0.0001 && candidate.direction === 'LONG') score += 5;
        }
