...
export class ChartGenerator {
    static async generateChart(ctx, signal) {
        const width = 800;
        const height = 400;
        const margin = { top: 30, right: 60, bottom: 30, left: 10 };
        // Preference for 15m candles to reduce noise, fallback to 5m
        let titleTimeframe = signal.timeframe;
        let chartCandles = ctx.candles;
        if (ctx.candles15m && ctx.candles15m.length > 0) {
            chartCandles = ctx.candles15m;
            titleTimeframe = '15m';
        }
        ...
        const scaleVol = (vol) => {
            const volHeight = drawHeight * 0.2; // Volume takes bottom 20%
        ...
        svg += `</svg>`;
        // Render SVG to PNG Buffer via sharp
